...
def write_csv(raw_info, processed_info, meta_data, csv_destination):
    with open(csv_destination, "w", newline="") as csvfile:
        ...
        writer.writerow(
            [
                "Beschrijving",
                "spreadsheet afwijkende percentages blanco en ongeldige stemmen, "
        ...
        for key in processed_info.keys():
            if check_any_true(processed_info[key]):
                result_row = create_result_row(
                    raw_info[key], processed_info[key], key.split("::SB")[1]
                )
                writer.writerow(result_row)


def write_afwijkeningen(meta_data, afwijkingen, raw_info, csv_destination):
    with open(csv_destination, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["sep=,"])
        writer.writerow(["Versie controleprotocol"])
